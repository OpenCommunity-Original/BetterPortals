...
/**
 * Tracks which portals are active/viewed each tick.
 *
 * <p>Uses two Set pairs per category: the "active" set and a "yet-to-update" set.
 * At the end of each tick, any portal still in "yet-to-update" was not visited and
 * gets deactivated. Then "yet-to-update" is reset to all currently-active portals
 * for the next tick.</p>
 *
 * <p>Compared with copying the whole set every tick via addAll/clear, this approach
 * is equivalent but avoids an extra allocation round-trip for the set wrapper.</p>
 */
@Singleton
