...
public class EventEntityTrackingManager extends EntityTrackingManager implements Listener {
    private final Map<Entity, List<IEntityTracker>> trackersByEntity = new java.util.concurrent.ConcurrentHashMap<>();
     ...
     */
    private final Map<Entity, EquipmentSlot> lastHandUsed = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    protected void newTrackerAdded(IEntityTracker tracker) {
        ...
        trackersByEntity.computeIfAbsent(tracker.getEntityInfo().getEntity(), entity -> new java.util.concurrent.CopyOnWriteArrayList<>()).add(tracker);
    }
